...
class DataBMS:
    ...
    def check(self):
        """Check bytes for correctness."""
        assert self.bytes[0] == 0xdd
        # assert (self.bytes[-3] << 8) + self.bytes[-2] == 0xffff - \
        #     sum(self.content) - self.length - self.status + 1
    ...
    def get_ros_message(self) -> BatteryState:
        """Get ROS message."""
        data = self.interpret()
        msg = BatteryState()
        msg.voltage = data['total voltage']
        msg.temperature = data['temperatures'][0]
        msg.capacity = data['residual capacity']
        msg.design_capacity = data['nominal capacity']
        msg.percentage = data['capacity percent'] / 100
